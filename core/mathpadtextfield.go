...
type MathpadTextField struct { //core:embedder
	...
	// will not be interactive. See [MathpadTextField.SetLeadingIcon].
	...
	// See [MathpadTextField.SetTrailingIcon].
	...
	// will not be interactive. See [MathpadTextField.SetTrailingIcon].
	...
	// see [MathpadTextField.SetTypePassword].
	NoEcho bool
	...
	// PlaceholderColor is the color used for the [MathpadTextField.Placeholder] text.
	// It should be set in a Styler like all other style properties.
	...
	// It must be set using [MathpadTextField.SetCompleter].
	complete *Complete
	...
	undos MathpadTextFieldUndos

	leadingIconButton, trailingIconButton *Button
}

// MathpadTextFieldTypes is an enum containing the
// different possible types of text fields.
type MathpadTextFieldTypes int32 //enums:enum -trim-prefix MathpadTextField

const (
	// MathpadTextFieldFilled represents a filled
	// [MathpadTextField] with a background color
	// and a bottom border.
	MathpadTextFieldFilled MathpadTextFieldTypes = iota

	// MathpadTextFieldOutlined represents an outlined
	// [MathpadTextField] with a border on all sides
	// and no background color.
	MathpadTextFieldOutlined
)

// MtValidator is an interface for types to provide a Validate method
// that is used to validate string [Value]s using [MathpadTextField.MtValidator].
type MtValidator interface {

	// Validate returns an error if the value is invalid.
	Validate() error
}

func (tf *MathpadTextField) WidgetValue() any { return &tf.text }

func (tf *MathpadTextField) OnBind(value any, tags reflect.StructTag) {
	if vd, ok := value.(MtValidator); ok {
		tf.MtValidator = vd.Validate
	}
}

func (tf *MathpadTextField) Init() {
	tf.Frame.Init()
	...
	// 	fmt.Println("mousedown", tf.selectInit)
	// })
	// tf.On(events.MouseUp, func(e events.Event) {
	// 	fmt.Println("mathpadtextfield mouseup at", e.Pos(), e.Pos().Sub(tf.Geom.contentRect().Min), tf.Geom.contentRect().Max.Sub(e.Pos()))
	// 	endpos := tf.pixelToCursor(e.Pos())
	// 	if tf.selectInit == endpos {
	// 		if e.Pos().Sub(tf.Geom.contentRect().Min).X > 2 && tf.Geom.contentRect().Max.Sub(e.Pos()).X > 2 {
	// 			//e.SetHandled()
	// 			tf.cursorPos = endpos
	// 			tf.updateLinePos()
	// 			tf.clearSelected()
	// 			tf.NeedsRender()
	// 			fmt.Println("1 tf.startCursor()")
	// 			tf.startCursor()
	// 		} else {
	// 			panic("error")
	// 		}
	...
	// 		fmt.Println("mouseup", endpos)
	// 		tf.dispRange.Start = tf.selectRange.Start
	// 		fmt.Println("tf.dispRange.Start", tf.dispRange.Start)
	// 		tf.dispRange.End = tf.selectRange.End
	// 		fmt.Println("tf.dispRange.End", tf.dispRange.End)
	// 		fmt.Println("tf.cursorPos", tf.cursorPos)
	// 		tf.updateCursorPosition()
	// 		fmt.Println("2 tf.startCursor()")
	// 		//tf.startCursor()
	// 		tf.NeedsRender()
	// 		fmt.Println("tf.cursorPos", tf.cursorPos)
	// 	}
...
func (tf *MathpadTextField) Destroy() {
	tf.stopCursor()
	tf.Frame.Destroy()
}

func (tf *MathpadTextField) selectToStartByPos(pos image.Point, curposAtLeft bool) {
	tf.selectInit = 0
	endpos := tf.pixelToCursor(pos)
	//if endpos != 0 {
	if curposAtLeft {
		tf.cursorPos = 0
	} else {
...
func (tf *MathpadTextField) selectToEndByPos(pos image.Point, curposAtLeft bool) {
	tf.selectInit = tf.pixelToCursor(pos)
...
func (tf *MathpadTextField) Text() string {
	tf.editDone()
	return tf.text
}
...
func (tf *MathpadTextField) SetText(text string) *MathpadTextField {
...
// SetLeadingIcon sets the [MathpadTextField.LeadingIcon] to the given icon. If an
// on click function is specified, it also sets the [MathpadTextField.LeadingIconOnClick]
// to that function. If no function is specified, it does not override any already
// set function.
func (tf *MathpadTextField) SetLeadingIcon(icon icons.Icon, onClick ...func(e events.Event)) *MathpadTextField {
	tf.LeadingIcon = icon
	if len(onClick) > 0 {
...
// SetTrailingIcon sets the [MathpadTextField.TrailingIcon] to the given icon. If an
// on click function is specified, it also sets the [MathpadTextField.TrailingIconOnClick]
// to that function. If no function is specified, it does not override any already
// set function.
func (tf *MathpadTextField) SetTrailingIcon(icon icons.Icon, onClick ...func(e events.Event)) *MathpadTextField {
...
// when it is clicked.
func (tf *MathpadTextField) AddClearButton() *MathpadTextField {
	return tf.SetTrailingIcon(icons.Close, func(e events.Event) {
		tf.clear()
	})
}

// SetTypePassword enables [MathpadTextField.NoEcho] and adds a trailing
// icon button at the end of the MathpadTextField that toggles [MathpadTextField.NoEcho].
// It also sets [styles.Style.VirtualKeyboard] to [styles.KeyboardPassword].
func (tf *MathpadTextField) SetTypePassword() *MathpadTextField {
...
func (tf *MathpadTextField) textEdited() {
	tf.edited = true
	tf.renderVisible = nil
	tf.NeedsRender()
}

// editDone completes editing and copies the active edited text to the [MathpadTextField.text].
// It is called when the return key is pressed or the text field goes out of focus.
func (tf *MathpadTextField) editDone() {
	if tf.edited {
		tf.edited = false
...
func (tf *MathpadTextField) revert() {
	tf.renderVisible = nil
	tf.editText = []rune(tf.text)
	tf.edited = false
	tf.dispRange.Start = 0
...
func (tf *MathpadTextField) clear() {
	tf.renderVisible = nil
	tf.edited = true
...
func (tf *MathpadTextField) clearError() {
	if tf.error == nil {
		return
	}
	tf.error = nil
...
// validate runs [MathpadTextField.MtValidator] and takes any necessary actions
// as a result of that.
func (tf *MathpadTextField) validate() {
	if tf.MtValidator == nil {
		return
	}
	err := tf.MtValidator()
	if err == nil {
...
func (tf *MathpadTextField) WidgetTooltip(pos image.Point) (string, image.Point) {
	if tf.error == nil {
...
func (tf *MathpadTextField) updateLinePos() {
	if tf.renderAll == nil {
		return
	}
...
func (tf *MathpadTextField) cursorForward(steps int) {
	tf.cursorPos += steps
...
func (tf *MathpadTextField) cursorForwardWord(steps int) {
...
func (tf *MathpadTextField) cursorBackward(steps int) {
	tf.cursorPos -= steps
	if tf.cursorPos < 0 {
		tf.cursorPos = 0
	}
...
func (tf *MathpadTextField) cursorBackwardWord(steps int) {
...
func (tf *MathpadTextField) cursorDown(steps int) {
	if tf.numLines <= 1 {
		return
	}
...
func (tf *MathpadTextField) cursorUp(steps int) {
	if tf.numLines <= 1 {
		return
	}
	if tf.cursorLine <= 0 {
...
func (tf *MathpadTextField) cursorStart() {
	tf.cursorPos = 0
	tf.dispRange.Start = 0
...
// if select mode is active.
func (tf *MathpadTextField) cursorEnd() {
	ed := len(tf.editText)
	tf.cursorPos = ed
...
func (tf *MathpadTextField) cursorBackspace(steps int) {
	if tf.hasSelection() {
		tf.deleteSelection()
		return
	}
...
func (tf *MathpadTextField) cursorDelete(steps int) {
	if tf.hasSelection() {
		tf.deleteSelection()
		return
	}
...
func (tf *MathpadTextField) cursorBackspaceWord(steps int) {
	if tf.hasSelection() {
		tf.deleteSelection()
		return
	}
...
func (tf *MathpadTextField) cursorDeleteWord(steps int) {
	if tf.hasSelection() {
		tf.deleteSelection()
		return
	}
...
func (tf *MathpadTextField) cursorKill() {
	steps := len(tf.editText) - tf.cursorPos
...
func (tf *MathpadTextField) selectModeToggle() {
	if tf.selectMode {
		tf.selectMode = false
	} else {
		tf.selectMode = true
...
func (tf *MathpadTextField) shiftSelect(e events.Event) {
...
func (tf *MathpadTextField) selectRegionUpdate(pos int) {
...
func (tf *MathpadTextField) selectAll() {
	tf.selectRange.Start = 0
	tf.selectInit = 0
...
func (tf *MathpadTextField) selectWord() {
	sz := len(tf.editText)
	if sz <= 3 {
		tf.selectAll()
		return
...
func (tf *MathpadTextField) selectReset() {
	tf.selectMode = false
...
func (tf *MathpadTextField) selectUpdate() {
...
func (tf *MathpadTextField) cut() { //types:add
	if tf.NoEcho {
		return
	}
...
func (tf *MathpadTextField) deleteSelection() string {
	tf.selectUpdate()
	if !tf.hasSelection() {
		return ""
	}
...
func (tf *MathpadTextField) copy() { //types:add
	if tf.NoEcho {
		return
	}
	tf.selectUpdate()
...
func (tf *MathpadTextField) paste() { //types:add
...
func (tf *MathpadTextField) insertAtCursor(str string) {
	if tf.hasSelection() {
		tf.cut()
	}
	rs := []rune(str)
...
// MathpadTextFieldUndoRecord holds one undo record
type MathpadTextFieldUndoRecord struct {
	text      []rune
	cursorPos int
}

func (ur *MathpadTextFieldUndoRecord) set(txt []rune, curpos int) {
	ur.text = slices.Clone(txt)
	ur.cursorPos = curpos
}

// MathpadTextFieldUndos manages everything about the undo process for a [MathpadTextField].
type MathpadTextFieldUndos struct {

	// stack of undo records
	stack []MathpadTextFieldUndoRecord
...
func (us *MathpadTextFieldUndos) saveUndo(txt []rune, curpos int) {
	n := len(us.stack)
	now := time.Now()
...
func (tf *MathpadTextField) saveUndo() {
	tf.undos.saveUndo(tf.editText, tf.cursorPos)
}

func (us *MathpadTextFieldUndos) undo(txt []rune, curpos int) *MathpadTextFieldUndoRecord {
	n := len(us.stack)
	if us.pos <= 0 || n == 0 {
		return &MathpadTextFieldUndoRecord{}
	}
	if us.pos == n {
...
func (us *MathpadTextFieldUndos) redo() *MathpadTextFieldUndoRecord {
	n := len(us.stack)
	if us.pos >= n-1 {
		return nil
	}
...
func (tf *MathpadTextField) SetCompleter(data any, matchFun complete.MatchFunc, editFun complete.EditFunc) {
...
func (tf *MathpadTextField) offerComplete() {
	if tf.complete == nil {
		return
	}
...
func (tf *MathpadTextField) cancelComplete() {
	if tf.complete == nil {
		return
	}
	tf.complete.Cancel()
...
func (tf *MathpadTextField) completeText(s string) {
...
func (tf *MathpadTextField) hasWordWrap() bool {
...
func (tf *MathpadTextField) relCharPos(st, ed int) math32.Vector2 {
...
func (tf *MathpadTextField) charRenderPos(charidx int) math32.Vector2 {
	pos := tf.effPos
	sc := tf.Scene
...
var (
	// MathpadTextFieldSpriteName is the name of the window sprite used for the cursor.
	MathpadTextFieldSpriteName = "MathpadTextField.Cursor"

	// MathpadTextFieldCursor is the MathpadTextField that last created a new cursor sprite.
	MathpadTextFieldCursor tree.Node
)

// startCursor starts the cursor blinking and renders it
func (tf *MathpadTextField) startCursor() {
...
func (tf *MathpadTextField) stopCursor() {
	tf.toggleCursor(false)
}

// toggleSprite turns on or off the cursor sprite.
func (tf *MathpadTextField) toggleCursor(on bool) {
...
func (tf *MathpadTextField) updateCursorPosition() {
	if tf.IsReadOnly() || !tf.StateIs(states.Focused) {
		return
	}
	sc := tf.Scene
...
func (tf *MathpadTextField) renderSelect() {
...
func (tf *MathpadTextField) autoScroll() {
	sz := &tf.Geom.Size
	icsz := tf.iconsSize()
...
func (tf *MathpadTextField) pixelToCursor(pt image.Point) int {
...
func (tf *MathpadTextField) setCursorFromPixel(pt image.Point, selMode events.SelectModes) {
	oldPos := tf.cursorPos
...
func (tf *MathpadTextField) Style() {
	tf.WidgetBase.Style()
	tf.CursorWidth.ToDots(&tf.Styles.UnitContext)
}

func (tf *MathpadTextField) configTextSize(sz math32.Vector2) math32.Vector2 {
	txt := tf.editText
	if len(txt) == 0 && len(tf.Placeholder) > 0 {
		txt = []rune(tf.Placeholder)
	}
	if tf.NoEcho {
		txt = mtConcealDots(len(tf.editText))
	}
...
func (tf *MathpadTextField) iconsSize() math32.Vector2 {
...
func (tf *MathpadTextField) SizeUp() {
	...
	if DebugSettings.LayoutTrace {
		fmt.Println(tf, "MathpadTextField SizeUp:", rsz, "Actual:", sz.Actual.Content)
	}
}
...
func (tf *MathpadTextField) SizeDown(iter int) bool {
	...
	if chg {
		if DebugSettings.LayoutTrace {
			fmt.Println(tf, "MathpadTextField Size Changed:", sz.Actual.Content, "was:", prevContent)
		}
	}
...
func (tf *MathpadTextField) SizeFinal() {
...
func (tf *MathpadTextField) ApplyScenePos() {
	tf.Frame.ApplyScenePos()
	tf.setEffPosAndSize()
}

// setEffPosAndSize sets the effective position and size of
// the MathpadTextField based on its base position and size
// and its icons or lack thereof
func (tf *MathpadTextField) setEffPosAndSize() {
...
func (tf *MathpadTextField) layoutCurrent() {
...
func (tf *MathpadTextField) Render() {
...
// mtConcealDots creates an n-length []rune of bullet characters.
func mtConcealDots(n int) []rune {
	dots := make([]rune, n)
	for i := range dots {
		dots[i] = '•'
	}
...
// SetNoEcho sets the [MathpadTextField.NoEcho]:
// NoEcho is whether replace displayed characters with bullets
// to conceal text (for example, for a password input). Also
// see [MathpadTextField.SetTypePassword].
func (t *MathpadTextField) SetNoEcho(v bool) *MathpadTextField { t.NoEcho = v; return t }

// AsMathpadTextField satisfies the [MathpadTextFieldEmbedder] interface
func (t *MathpadTextField) AsMathpadTextField() *MathpadTextField { return t }

// SetType sets the [MathpadTextField.Type]:
// Type is the styling type of the text field.
func (t *MathpadTextField) SetType(v MathpadTextFieldTypes) *MathpadTextField {
	t.Type = v
	return t
}

// SetPlaceholder sets the [MathpadTextField.Placeholder]:
// Placeholder is the text that is displayed
// when the text field is empty.
func (t *MathpadTextField) SetPlaceholder(v string) *MathpadTextField {
	t.Placeholder = v
	return t
}

// SetValidator sets the [MathpadTextField.MtValidator]:
// MtValidator is a function used to validate the input
// of the text field. If it returns a non-nil error,
// then an error color, icon, and tooltip will be displayed.
func (t *MathpadTextField) SetValidator(v func() error) *MathpadTextField {
	t.MtValidator = v
	return t
}

// SetLeadingIconOnClick sets the [MathpadTextField.LeadingIconOnClick]:
// LeadingIconOnClick, if specified, is the function to call when
// the LeadingIcon is clicked. If this is nil, the leading icon
// will not be interactive. See [MathpadTextField.SetLeadingIcon].
func (t *MathpadTextField) SetLeadingIconOnClick(v func(e events.Event)) *MathpadTextField {
	t.LeadingIconOnClick = v
	return t
}

// SetTrailingIconOnClick sets the [MathpadTextField.TrailingIconOnClick]:
// TrailingIconOnClick, if specified, is the function to call when
// the TrailingIcon is clicked. If this is nil, the trailing icon
// will not be interactive. See [MathpadTextField.SetTrailingIcon].
func (t *MathpadTextField) SetTrailingIconOnClick(v func(e events.Event)) *MathpadTextField {
	t.TrailingIconOnClick = v
	return t
}

// SetCursorWidth sets the [MathpadTextField.CursorWidth]:
// CursorWidth is the width of the text field cursor.
// It should be set in a Styler like all other style properties.
// By default, it is 1dp.
func (t *MathpadTextField) SetCursorWidth(v units.Value) *MathpadTextField {
	t.CursorWidth = v
	return t
}

// SetCursorColor sets the [MathpadTextField.CursorColor]:
// CursorColor is the color used for the text field cursor (caret).
// It should be set in a Styler like all other style properties.
// By default, it is [colors.Scheme.Primary.Base].
func (t *MathpadTextField) SetCursorColor(v image.Image) *MathpadTextField {
	t.CursorColor = v
	return t
}

// SetPlaceholderColor sets the [MathpadTextField.PlaceholderColor]:
// PlaceholderColor is the color used for the [MathpadTextField.Placeholder] text.
// It should be set in a Styler like all other style properties.
// By default, it is [colors.Scheme.OnSurfaceVariant].
func (t *MathpadTextField) SetPlaceholderColor(v image.Image) *MathpadTextField {
	t.PlaceholderColor = v
	return t
}

// NewMathpadTextField returns a new [MathpadTextField] with the given optional parent:
